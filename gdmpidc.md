...
$$ \frac{R_\mathrm{c}}{R_\mathrm{s}} = \left(\frac{1}{\eta_\mathrm{c}}\right)^{1/3} $$
...
$$ \rho(r) = \rho_0 \exp\left[-\frac{(r - \mu R_\mathrm{c})^2}{2\sigma^2 R_\mathrm{c}^2}\right] $$
...
$$ M_\mathrm{c} = 4\pi \int_{0}^{R_\mathrm{c}} \rho r^2 dr $$
...
$$ \eta_\mathrm{c}(r) = \frac{\rho(r)}{\rho_{\mathrm{max}}} \cdot \eta_{\mathrm{max}} $$
...
$$ \bar{v}_{\mathrm{explosion}} = 0.2\log \frac{A}{M} + 1.85 $$

$$ \bar{v}_{\mathrm{collision}} = 0.9\log \frac{A}{M} + 2.9 $$
...
$$ \sigma(L_\mathrm{c}) \simeq \sigma_0 L_\mathrm{c}^{-\alpha} $$
...
$$ M_{\mathrm{total}} = 4\pi \int_{L_{\mathrm{min}}}^{L_{\mathrm{max}}} \int_{0}^{R_\mathrm{c}} r^2 \rho(r, L_\mathrm{c})dr, n(L_\mathrm{c})dL_\mathrm{c} $$
...
$$ N_{\mathrm{explosion}}(L_\mathrm{c}) = 6L_\mathrm{c}^{-1.6} $$

$$ N_{\mathrm{collision}}(L_\mathrm{c}) = 0.1L_\mathrm{c}^{-1.71} M_{\mathrm{parent}}^{0.75} $$
...
$$ R_\mathrm{c}(t) = R_\mathrm{c}(0) + v_{\mathrm{expansion}} \cdot t $$
...
$$ \rho_0(L_\mathrm{c}, t) = \rho_0(L_\mathrm{c}, 0)\left[\frac{R_\mathrm{c}(0)}{R_\mathrm{c}(t)}\right]^3 $$
...
$$ \bar{v}_{\mathrm{expansion}} = \frac{\int_{L_\mathrm{c}} \bar{v}n, dL_\mathrm{c}}{\int_{L_\mathrm{c}} n, dL_\mathrm{c}} $$
...
$$ P(\vec{r}|L_\mathrm{c}, t) = \frac{\rho(|\vec{r}|, L_\mathrm{c}, t)}{4\pi\int_0^{R_\mathrm{c}(t)} \rho(r, L_\mathrm{c}, t)r^2, dr} $$
...
$$ \vec{v}_{\mathrm{fragment}} = \vec{v}_{\mathrm{parent}} + \vec{v}_{\mathrm{ejection}} $$
...
$$ \vec{\ddot{r}}_{\mathrm{grav}} = -\frac{GM_\oplus}{r^2}\hat{r} $$
...
$$ \vec{a}_{\mathrm{total}} = \vec{\ddot{r}}_{\mathrm{grav}} + \vec{a}_{\mathrm{pert}} $$
...
$$ \vec{a}_{\mathrm{pert}} \simeq \frac{k_{\mathrm{pert}}}{L_\mathrm{c}} $$
...
$$ \vec{a}_{\mathrm{pert}} = \vec{a}_{\mathrm{SRP}} + \vec{a}_d + \vec{a}_{\mathrm{EM}} $$
...
$$ \vec{a}_{\mathrm{SRP}} = -P_{\mathrm{SRP}}\frac{A}{M}C_R\hat{r}_\odot $$
...
$$ \vec{a}_{\mathrm{SRP}} \simeq -P_{\mathrm{SRP}}\frac{k}{L_\mathrm{c}}C_R\hat{r}_\odot $$
...
$$ \vec{a}_d = -\frac{1}{2}\rho_{\mathrm{atm}}\frac{A}{M}C_d v_{\mathrm{rel}}^2\hat{v}_{\mathrm{rel}} $$

$$ \simeq -\frac{1}{2}\rho_{\mathrm{atm}}\frac{k}{L_\mathrm{c}}C_d v_{\mathrm{rel}}^2\hat{v}_{\mathrm{rel}} $$
...
$$ \sigma(L_\mathrm{c}, t) = \sigma(L_\mathrm{c}, 0) + \gamma, t L_\mathrm{c}^{-\alpha} $$
...
$$ \rho(r, L_\mathrm{c}, t) = \rho_0(L_\mathrm{c}, t) \exp\left[-\frac{1}{2}\left(\frac{r - \mu(L_\mathrm{c})R_\mathrm{c}(t)}{\sigma(L_\mathrm{c}, t)R_\mathrm{c}(t)}\right)^2\right] $$
...
$$ 4\pi \int_0^{R_\mathrm{c}(t)} P(r|L_\mathrm{c}, t)r^2 dr = 1 $$

For the probability to be proportional to the mass density, dividing $\rho$ by the total mass normalizes this distribution:

$$ P(\vec{r}|L_\mathrm{c}, t) = \frac{\rho(|\vec{r}|, L_\mathrm{c}, t)}{M_{\mathrm{total}}(t)} $$

where,

$$ M_{\mathrm{total}}(t) = 4\pi \int_0^{R_\mathrm{c}(t)} \rho(r, L_\mathrm{c}, t)r^2 dr $$

## Appendix B: Size Classification

Size classifications in this model are as follows:

-   Small ($L_\mathrm{c} < 1$ cm); experience rapid dispersion due to dominant SRP and atmospheric drag effects [3, 8]
-   Medium (1 cm < $L_\mathrm{c}$ < 10 cm); experience moderate deviation primarily due to SRP [3]
-   Large ($L_\mathrm{c} > 10$ cm); remain close to their initial ballistic trajectories [1]
