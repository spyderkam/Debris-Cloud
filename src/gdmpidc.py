...
if __name__ == "__main__":
  ...
  in_ratios_s, out_ratios_s = [], []     # inside and outside probabilities of small fragments
  ...
  for size_category in fragment_data.keys():
    ...
    if size_category == "small":
      print(f"Small fragments: {np.mean(in_ratios)*100}% inside")
    elif size_category == "medium":
      print(f"Medium fragments: {np.mean(in_ratios)*100}% inside")
    else:
      print(f"Large fragments: {np.mean(in_ratios)*100}% inside")
